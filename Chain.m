classdef Chain
    ...
    methods
        function obj = Chain(id, sem_prio)
            ...
            if length(obj.sem_priority) == 0 || obj.sem_priority < sem_prio
        ...
        function obj = add_callback(obj, callback)
            ...
            for c = 1 : length(obj.t_callback)
                ...
                obj.t_callback(c).chain_T = obj.T;
            end
            for c = 1 : length(obj.r_callbacks)
                obj.r_callbacks(c).chain_c = obj.C;
                obj.r_callbacks(c).chain_T = obj.T;
            end
            if length(obj.sem_priority) == 0 || obj.sem_priority < callback.priority
                obj.sem_priority = callback.priority;
            end
        end
    end
